...
export default function ContainerInfo(props: ContainerProps): JSX.Element {
  ...
  const [latestOfficialBlockHeight, setLatestOfficialBlockHeight] =
    createSignal(0);
  ...
  if (role === "Validator") {
    return renderContainerInfo({
      ...
      netout_rate,
      blockHeight,
      latestOfficialBlockHeight,
    });
  }

  const statusClass = props.container.status === "running"
    ? "bg-hex-AECE4B"
    : "bg-red";
...
function renderContainerInfo(
  ...
  return (
    <div class="font-mono p-4 border my-4 filter-drop-shadow bg-hex-dfe9c5 rounded shadow-sm text-hex-010001">
      ...
      <p>
        cpu usage: <UsageBar current={cpu} max={1} />
      </p>
      <p>
        memory used: <UsageBar current={memory_used} max={memory_total} />
      </p>
